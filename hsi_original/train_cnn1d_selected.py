...
# =============================
# 🚀 1D-CNN Training — Selected Model (with band selection)
# =============================
...
# =============================
print("📥 Loading data...")
X = np.load(os.path.join(DATA_PATH, 'X_selected.npy'))
...
checkpoint = ModelCheckpoint(
    os.path.join(DATA_PATH, 'model_cnn1d_selected.keras'),
    monitor="val_loss",
    save_best_only=True,
    verbose=1
)
...
# =============================
final_model_path = os.path.join(DATA_PATH, 'model_cnn1d_selected_final.keras')
...
with open(os.path.join(DATA_PATH, 'history_selected.pkl'), 'wb') as f:
    pickle.dump(hist_dict, f)
with open(os.path.join(DATA_PATH, 'history_selected.json'), 'w') as f:
    json.dump(hist_dict, f, indent=2)
pd.DataFrame(hist_dict).to_csv(os.path.join(DATA_PATH, 'history_selected.csv'), index=False)
print("🧾 History saved as history_selected.pkl / .json / .csv")
...
plt.title('Loss Curve — 1D-CNN (Selected Bands)')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
...
plt.title('Accuracy Curve — 1D-CNN (Selected Bands)')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.grid(True, linestyle='--', linewidth=0.5)
...
plt.tight_layout()
la_path = os.path.join(DATA_PATH, "loss_accuracy_selected.png")
...
plt.tight_layout()
cm_counts_path = os.path.join(DATA_PATH, 'confusion_matrix_selected_counts.png')
...
plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.5)
plt.tight_layout()
cm_norm_path = os.path.join(DATA_PATH, 'confusion_matrix_selected_normalized_pct.png')
...
plt.title('ROC Curve — 1D-CNN (Selected Bands)')
plt.legend(loc="lower right")
plt.grid(True, linestyle='--', linewidth=0.5)
plt.tight_layout()
roc_path = os.path.join(DATA_PATH, 'roc_curve_selected.png')
